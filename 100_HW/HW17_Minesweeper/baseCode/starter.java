...
class starter{
    ...
    */
    public static void mapSetUp(int diff, int [][] bombProxGrid){
        ...
        if (diff == 1) {
            y = 10;
            while (y > 0){
                for (int o = 0; o < bombProxGrid.length; o++){
                    for (int p = 0; p < bombProxGrid[0].length; p++){
                        if (bombProxGrid[o][p] != 9) {
                            if (x == ((int) (Math.random()*9 + 1))) {
                                bombProxGrid[o][p] = 9;
                                addAdj(o, p, bombProxGrid);
                                y--;
                            }
                        }
                    }
    ...
    public static void addAdj(int r, int c, int [][] bombProxGrid){
        for (int i = r - 1; i <= r + 1; i++) {
            for (int o = c - 1; o <= c + 1; o++) {
                if ((i >= 0) && (i < bombProxGrid.length) && (o >= 0) && (o < bombProxGrid[0].length) && (bombProxGrid[i][o] != 9)) {
                    bombProxGrid[i][o] = bombProxGrid[i][o] + 1;
                }
            }
        }
    }
    ...
    */
    public static void reveal(int r, int c, int [][] guessGrid, int[][] bombProxGrid){
        for (int i = r - 1; i <= r + 1; i++) {
            for (int o = c - 1; o <= c + 1; o++) {
                if ((i >= 0) && (i < bombProxGrid.length) && (o >= 0) && (o < bombProxGrid[0].length)) {
                    guessGrid[i][o] = bombProxGrid[i][o];
                }
            }
        }
        printGrid(guessGrid);
    ...
    */
    public static boolean checkWin(int [][] guessGrid, int [][] bombProxGrid){
        for (int i = 0; i < guessGrid.length; i++) {
            for (int o = 0; o < guessGrid[0].length; o++) {
                if (guessGrid[i][o] != bombProxGrid[i][o]) {
                    return false;
                }
            }
        }
